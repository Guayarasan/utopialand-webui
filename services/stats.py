...
@cached(Config.CACHE_TTL_SECONDS)
def get_activity_timeseries(days=30, tz_offset=None):
    """
    ...
    la tabla. Se convierte a `tz_offset` (offset '+HH:MM' de la zona
    horaria de visualización) antes de extraer el día -- si no, MySQL
    trocea los días según la zona horaria de SU sesión, no la elegida en
    la WebUI.
    """
    ...
    sql = f"""
        SELECT
            DATE(CONVERT_TZ(FROM_UNIXTIME(time), '+00:00', %s)) AS day,
            COUNT(*) AS total
        FROM {TABLE}
        WHERE time >= %s
        GROUP BY day
        ORDER BY day ASC
    """
    rows = fetch_all(sql, [tz_offset, since])
    return [{"day": str(r["day"]), "total": r["total"]} for r in rows]


@cached(Config.CACHE_TTL_SECONDS)
def get_hourly_activity(days=30, tz_offset=None):
...
@cached(Config.CACHE_TTL_SECONDS)
def get_heatmap(days=30, tz_offset=None):
    """
    Mapa de calor día-de-semana x hora, últimos N días, en la zona
    horaria de visualización. Devuelve una matriz plana
    [{weekday, hour, total}] -- weekday 0=lunes..6=domingo (se normaliza
    el WEEKDAY() de MySQL, que ya usa ese orden).
    """
    tz_offset = tz_offset or current_utc_offset_sql()
    since = now_unix() - days * 86400
    sql = f"""
        SELECT
            WEEKDAY(CONVERT_TZ(FROM_UNIXTIME(time), '+00:00', %s)) AS weekday,
            HOUR(CONVERT_TZ(FROM_UNIXTIME(time), '+00:00', %s)) AS hour,
            COUNT(*) AS total
        FROM {TABLE}
        WHERE time >= %s
        GROUP BY weekday, hour
    """
    rows = fetch_all(sql, [tz_offset, tz_offset, since])
